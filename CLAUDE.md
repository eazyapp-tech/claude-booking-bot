...
**CI security gate** (`.github/workflows/ci.yml`): hermetic suite — `test_untrusted_content.py`, `test_tenant_isolation.py`, `test_webhook_signature.py`, `test_cost_accounting.py`, `test_server_stop.py`, `test_engine_contract.py`, `test_tool_boundary.py`, `test_wave_a.py`, `test_contract_alignment.py` (9 tests, 201 assertions, no network/Redis/LLM). Reusable workflow (`pull_request` + `workflow_call`). On PRs it is the required status check (configure branch protection to block merge on failure). `deploy-render.yml`'s `deploy` job `needs: security-tests` (calls `ci.yml` via `workflow_call`) so a red suite blocks the Render deploy — the suite runs once per commit, not twice.
