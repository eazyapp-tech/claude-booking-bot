...
core/signal_extractor.py (580) — Inference-based lead signal extraction (no LLM) | Pure extraction functions: extract_urgency@, extract_decision_authority@, extract_topic_frequency@, extract_lifestyle_tags@, extract_roommate_preferences@ (veg/smoking/professional/no-curfew/regional/student-friendly flags), extract_amenity_preferences@ (must-have/nice-to-have/deal-breaker tone detection + freq escalation ≥3 turns), infer_needs_from_lifestyle@ (lifestyle_tag→implied needs), _extract_amenity_tone@ (sentence-level tone×amenity co-occurrence). merge_topic_frequency@, merge_lifestyle_tags@. Constants: LIFESTYLE_TO_NEEDS, NEED_TO_AMENITY, _AMENITY_PATTERNS (17 amenities), _MUST/_NICE/_DEALBREAKER_PATTERNS, _ROOMMATE_PATTERNS (7 flags). dominant_intent_from_topics@ used by classify_intent() Tier 3. extract_and_update@ (async I/O wrapper) writes 9 user_memory fields: urgency/decision_authority/lifestyle_tags/inferred_needs/topic_frequency/roommate_prefs/inferred_must_haves/nice_to_haves/amenity_freq. Runs as asyncio.create_task in pipeline.py after every agent response. Never raises.
...
db/redis/user.py     (455)  — User memory, preferences, shortlist, followups, lead score, name, phone | get_user_memory@12, update_user_memory@40, get_lead_score@200, schedule_followup@280, add_deal_breaker@. _MEMORY_DEFAULTS includes 9 signal-extractor fields: urgency, decision_authority, lifestyle_tags, inferred_needs (list derived from lifestyle), topic_frequency, roommate_prefs (dict[str,bool]: veg_only/no_smoking/professional_only/student_friendly/no_curfew/regional_preference/non_veg_ok), inferred_must_haves (list), nice_to_haves (list), amenity_freq (dict[amenity,int]). build_returning_user_context@ surfaces all 9 in broker prompt. Forward-compat: get_user_memory merges _MEMORY_DEFAULTS so old records grow new keys automatically.
