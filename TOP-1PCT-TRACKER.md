...
| **SIG-1** | Inference-based lead signal extraction (no LLM). `core/signal_extractor.py` extracts urgency, decision_authority, lifestyle_tags → inferred_needs, topic_frequency, roommate_prefs, amenity tone (must-have / nice-to-have / deal-breaker with frequency escalation). Fire-and-forget via `asyncio.create_task` in pipeline.py. Outputs surface in `build_returning_user_context` + feed tier-3 behavioral intent in `classify_intent`. 98/98 hermetic. | Rf Rl | ✅ | local `f6b606e` |
| **DX-1** | Surface electricity + food inclusion flags from microsite API (`is_electricity_included`, `is_food_included`) as `'Yes'/'No'` in property details output. ~60% of OxOtel properties have these set (almost all true). `_bool_to_yes_no` helper + 19/19 hermetic. | H Rl | ✅ | local `d4c1ce0` |
| **DX-5** | Broker acts on inferred signals. `_base.md` `<cross_session_intelligence>` extended with instructions for urgency (URGENT → straight to properties), family authority (no hard closes, offer shareable summary), lifestyle matches (natural 1-line callouts — "this one has parking, I know that matters"), roommate prefs (only confirm what property data supports). | Rl | ✅ | local `6f4b19e` |
...
- **2026-06-08 (SIG-1 + DX-1 + DX-5 + Orchid Parc brand)** — Three new bot-only items plus brand seeding.
  **Orchid Parc brand:** DB query confirmed 3 Firebase pg_ids (198 Bengaluru suite listings under one pg_id, 1 standalone Bengaluru, 1 Gurugram). Brand config created live via POST /admin/brand-config with key `OrchidParc1234`; chatbot URL = `https://eazypg-chat.vercel.app?brand=16b842de-a833-4a09-b5fe-6ce8bab661d5`; seeded into `_SEED_BRANDS` in `main.py` with locked token so it survives Redis resets.
  **SIG-1 (signal extractor — from prior session, committed this session):** `core/signal_extractor.py` — pure hermetic inference from conversation history. Extracts: urgency (immediate/near/flexible), decision_authority (self/family), lifestyle_tags (night_schedule/has_vehicle/values_privacy/student) → inferred_needs, topic_frequency (6 topics), roommate_prefs (7 flags), amenity tone (must-have/nice-to-have/deal-breaker + 3-mention escalation). Wired fire-and-forget into `pipeline.py`. New memory fields in `db/redis/user.py`; `build_returning_user_context` surfaces urgency label, family advisory, roommate flags, lifestyle tags, inferred amenity needs. `classify_intent` gains a tier-3 behavioral tiebreaker via topic_frequency. `search.py` merges inferred must-haves + nice-to-haves into scoring. 98/98 hermetic. Gate 44/44.
  **DX-1:** `tools/broker/property_details.py` + `_fetch_details_raw` now extract `is_electricity_included` and `is_food_included` from the microsite API response (ms-first, pd fallback) → `'Yes'/'No'/''`. Empty when absent so no regression on unconfigured properties. 19/19 hermetic. Gate 45/45.
  **DX-5:** `skills/broker/_base.md` `<cross_session_intelligence>` extended with 4 inferred-signal blocks — urgency (URGENT → skip warm-up, straight to properties), family authority (no hard closes, shareable summary offer), lifestyle + implied needs (natural 1-line property callouts by tag), roommate prefs (only surface what property data confirms, never invent). Broker already receives all signals via `{returning_user_context}`; these instructions tell it what to DO with them without awkwardly naming the internal labels.
  **Open:** DX-2 (verified badge), DX-3 (rich amenity breakdown), DX-4 (amenity key mismatch in scoring cache) — all bot-only, no blockers.
